...
#  ymax <- 212
  ...
  for (yr in str_yr:end_yr){ #doesn't iterate yr 2018 because when iterating 2017, 2018 will be counted in
    ...
    for (aoi in 1:length(allaoi)){
        ...
        for (month in 1:12){ #12 months
          ...
          if (length(subset_aoiimages)==0){
            ...
            if (dim(NDVIstack)[3]!=1){
              ...
              #if GRVIstack has more than one layer 
            ...
            }else if (dim(GRVIstack)[3]==1){ 
              # if GRVIstack only has one layer, meaning there is only one available image for the month, 
      ...
      wrk_xyid <- as.character(allaoi[aoi]) 
      if (yr==2017){
        writeRaster(outputraster,paste(image_path,"/2017and2018_",wrk_xyid,".tif",sep=""),overwrite=TRUE)

        #png(outputraster, paste(image_path,"/2017and2018/",wrk_xyid,".png",sep=""))
      }else{
        writeRaster(outputraster,paste(image_path,"/",wrk_xyid,".tif",sep=""),overwrite=TRUE)
       
       # png(outputraster, paste(image_path,"/",wrk_xyid,".png",sep=""))
      }
      #plot(outputraster,
      #     col=colors)
      #dev.off()
      
      print(paste("please check the image file:", image_path,"/",wrk_xyid,".png",sep="") )
      # print current time
      print(Sys.time())
